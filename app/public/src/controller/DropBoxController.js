class DropBoxController {
    ...
    constructor() {
        ...
        this.snackModalInputEl = document.querySelector('#react-snackbar-root');
        this.progressBarEl = this.snackModalInputEl.querySelector('.mc-progress-bar-fg');
        this.fileNameEl = this.snackModalInputEl.querySelector('.filename');
        this.timeleftEl = this.snackModalInputEl.querySelector('.timeleft');
    ...
    connectFirebase() {
        var firebaseConfig = {
            apiKey: "AIzaSyAXmVE1YR7UJsEBR7l7z7Kpt29RCOFHX68",
            authDomain: "dropbox-clone-91a6a.firebaseapp.com",
            databaseURL: "https://dropbox-clone-91a6a.firebaseio.com",
            projectId: "dropbox-clone-91a6a",
            storageBucket: "dropbox-clone-91a6a.appspot.com",
            messagingSenderId: "229510930239",
            appId: "1:229510930239:web:7e9b8f3152d6670d2efd8a",
            measurementId: "G-4668Z6SGJN"
        };
    ...
    //metodo usado para remover do servidor express
    removeTaskAjax() {
        let promises = [];
    ...
    initEvents() {
        ...
        this.inputFileEl.addEventListener('change', event => {
            ...
            this.uploadTask(event.target.files).then(responses => {
                ...
                //utilizado quando é chamado pelo metodo taskAjax
                ...
                responses.forEach(res => {
                    ...
                    console.log(res.customMetadata.downloadURL);

                    this.getFirebaseRef().push().set({
                        name: res.name,
                        type: res.contentType,
                        path: res.customMetadata.downloadURL,
                        size: res.size
                    });
                });
    ...
    showModal(show = true) {
        ...
        this.snackModalInputEl.style.display = (show) ? 'block' : 'none';

    }


    ajax(url, method = 'GET', formData = new FormData(), onprogress = function () { }, onstartime = function () { }) {
    ...
    // metodo para enviar para o servidor express
    uploadTaskAjax(files) {

        let promises = [];

        [...files].forEach(file => {

            //API PRA envio de file para o servidor node.js
            let formData = new FormData();
    ...
    uploadProgress(event, file) {
        ...
        this.timeleftEl.innerHTML = this.formatTimeToHumanDuration(timeLeft);

    }

    formatTimeToHumanDuration(duration) {
    ...
    initEventsLi(li) {
        ...
        li.addEventListener('dblclick', e => {
            ...
            switch (file.type) {
                ...
                //usado para pegar o arquivo no servidor express
        ...
        li.addEventListener('click', e => {
            ...
            if (!e.ctrlKey) {
                ...
                this.listFilesEl.querySelectorAll('li.selected').forEach(e => {
    ...
    ReadFiles() {
        ...
        this.getFirebaseRef().on('value', snapchot => {

            this.listFilesEl.innerHTML = '';
            snapchot.forEach(snapchotItens => {
                let key = snapchotItens.key;

                let data = snapchotItens.val();

                let li = this.getFileView(data, key);
